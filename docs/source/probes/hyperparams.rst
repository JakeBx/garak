...
6. A sidecar ``<run>.hyperparam_index.json`` file is written alongside the
   JSONL report, mapping each parameter combo to the attempt UUIDs that used
   it. After the run completes, use ``garak.analyze.hyperparam_summary`` to
   produce the per-combo pass/fail table (see `Viewing results`_ below).
...
A sidecar ``<run>.hyperparam_index.json`` is written alongside the JSONL
report for reference; it maps each parameter combination to the list of
attempt UUIDs that used it.
